...
def _update_rate_stub(frm: str, to: str, now: datetime, rates: dict) -> tuple[float, str]:
    exchange_rates_stub = {
        "EUR_USD": 1.0786,
        "BTC_USD": 59337.21,
        "RUB_USD": 0.01016,
    ...
    rates[f"{frm}_{to}"] = {"rate": rate, "updated_at": updated_iso}
    rates["source"] = "Stub"
...
def show_portfolio(user_id: int, base: str | None = None) -> str:
    ...
    # имя пользователя для заголовка
    ...
    # читаем курсы из кеша (или пусто)
    rates = db.read("RATES_JSON", default={})

    def _get_rate_to_base_local(code: str, base_code: str) -> float:
        if code == base_code:
            return 1.0
        pair = f"{code}_{base_code}"
        pair_data = rates.get(pair)
        if not isinstance(pair_data, dict) or "rate" not in pair_data:
            # по ТЗ: "Неизвестная базовая валюта 'ABC'"
            raise ValueError(f"Неизвестная базовая валюта '{base_code}'")
        rate = pair_data["rate"]
    ...
    for code in codes_sorted:
        ...
        rate = _get_rate_to_base_local(code, base)
...
@log_action("BUY")
def buy(user_id: int, currency_code: str, amount: float, base: str | None = None) -> str:
    ...
    return (
        f"Покупка выполнена: {fmt_bal(code, float(amount))} {code} по курсу {rate:,.2f} {base_code}/{code}\n"
        f"Изменения в портфеле:\n"
        f"- {code}: было {fmt_bal(code, old_balance)} → стало {fmt_bal(code, new_balance)}\n"
        f"Оценочная стоимость покупки: {estimated_cost:,.2f} {base_code}"
    )


@log_action("SELL")
def sell(user_id: int, currency_code: str, amount: float, base: str | None = None) -> str:
    if not isinstance(amount, (int, float)) or float(amount) <= 0:
        raise ValueError("'amount' должен быть положительным числом")
    ...
    return (
        f"Продажа выполнена: {fmt_bal(code, float(amount))} {code} по курсу {rate:,.2f} {base_code}/{code}\n"
        f"Изменения в портфеле:\n"
        f"- {code}: было {fmt_bal(code, old_balance)} → стало {fmt_bal(code, new_balance)}\n"
...
def get_rate(from_code: str, to_code: str) -> tuple[float, str]:
    ...
    rate, updated_iso = _update_rate_stub(frm, to, now, rates)
    db.write("RATES_JSON", rates)
    return rate, updated_iso
